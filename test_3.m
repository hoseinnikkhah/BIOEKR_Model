...
for i = 2:Nx-1
    ...
    C(i,i-1) = -D(i,3)*(1/dx^2);
    C(i,i) = (1/dt) + D(i,3)*(2/dx^2) + v(i,3);
    C(i,i+1) = -D(i,3)*(1/dx^2);
    
    D(i,i-1) = (1/dx)*F*z(1)*D(i,1);
    D(i,i) = (1/dx)*F*z(2)*D(i,2);
    D(i,i+1) = (1/dx)*F*z(3)*D(i,3);
    
    E(i,i-1) = (1/dx)*F*z(1)*u(i,1)*C(i,1);
    E(i,i) = (1/dx)*F*z(2)*u(i,2)*C(i,2);
    E(i,i+1) = (1/dx)*F*z(3)*u(i,3)*C(i,3);
end
...
B(1,1) = 1;
C(1,1) = 1;
D(1,1) = F*z(1)*D(1,1)/dx;
D(1,2) = F*z(2)*D(1,2)/dx;
D(1,3) = F*z(3)*D(1,3)/dx;
E(1,1) = F*z(1)*u(1,1)*C(1,1)/dx;
E(1,2) = F*z(2)*u(1,2)*C(1,2)/dx;
E(1,3) = F*z(3)*u(1,3)*C(1,3)/dx;

A(Nx,Nx) = 1;
B(Nx,Nx) = 1;
C(Nx,Nx) = 1;
D(Nx,Nx) = F*z(1)*D(Nx,1)/dx;
D(Nx,Nx-1) = -F*z(2)*D(Nx,2)/dx;
D(Nx,Nx-2) = F*z(3)*D(Nx,3)/dx;
E(Nx,Nx) = F*z(1)*u(Nx,1)*C(Nx,1)/dx;
E(Nx,Nx-1) = -F*z(2)*u(Nx,2)*C(Nx,2)/dx;
E(Nx,Nx-2) = F*z(3)*u(Nx,3)*C(Nx,3)/dx;
