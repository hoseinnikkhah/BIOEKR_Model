...
% $$$
% Total Flux arrays [Bio]
J_HA_bio_alt = zeros(nx,nt);
J_OH_bio_alt = zeros(nx,nt);
J_Na_bio_alt = zeros(nx,nt);
J_Cl_bio_alt = zeros(nx,nt);
J_A_bio_alt = zeros(nx,nt);
J_H_bio_alt = zeros(nx,nt);
J_C_bio_alt = zeros(nx,nt);
...
for m=1:nt-1
    ...
    for i=2:nx-1
        ...
        % This is for total BKR [Standard]
        G_HA_bio_alt(i,m+1) = G_HA_bio_alt(i,m) + growth_alt(i,m)*(coeff*(((D_HA_day*h2)/(n*(tau^2)))*(G_HA_bio_alt(i+1,m) -2*G_HA_bio_alt(i,m) + G_HA_bio_alt(i-1,m)) - (h1/n)*((u_t_HA(i+1,m) - u_t_HA(i,m))*(G_HA_bio_alt(i+1,m) - G_HA_bio_alt(i,m)))));
        G_Na_bio_alt(i,m+1) = G_Na_bio_alt(i,m) + growth_alt(i,m)*(coeff*(((D_Na_day*h2)/(n*(tau^2)))*(G_Na_bio_alt(i+1,m) -2*G_Na_bio_alt(i,m) + G_Na_bio_alt(i-1,m)) - (h1/n)*((u_t_Na(i+1,m) - u_t_Na(i,m))*(G_Na_bio_alt(i+1,m) - G_Na_bio_alt(i,m)))));
