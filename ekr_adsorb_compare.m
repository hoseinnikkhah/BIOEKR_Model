...
for xx = 1:nx
    for tt = 1:nt
        M_g = exp(-K*(tt/1440));
        sub(xx,tt) = M_g;
        fixup(xx,tt) = 1/(-0.01*(tt/1440) + 1.21);
    end
end
growth = sub.*fixup;
...
plot(t, G_C_B(10,:), '--', 'DisplayName', 'Hydrocarbon (BKR) without adsorb term');
...
%figure(2);
%fig = plot(pH_scale, pH(:,1));
%xlabel('Distance (cm)'); % Corrected the misspelling from 'xlable' to 'xlabel'
%ylabel('pH Level');

%for h = 1:50400
    %set(fig, 'XData', pH_scale, 'YData', pH(:,h)');
    %h2 = h/1440;
    %title(['pH (EKR) - Day: ', sprintf('%.2f', h2)]);
    %pause(0.001); 
%end
...
plot(pH_scale,pH_B(:,50400),'-','DisplayName', 'pH (EKR) without adsorb term')
xlabel('Length (cm)');
ylabel('pH level');
legend();
