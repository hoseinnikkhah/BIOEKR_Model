...
% $$$
% Geomesh info
L = 0.2;                       % length of domain in x direction [m]

% $$$
% Nodes
tmax = 25;                     % end time [day]
nx = 21;                       % number of nodes in x direction
nt = 36001;                    % number of time steps
dx = L/(nx-1);
dt = tmax/(nt-1);
...
API = API_csv;
mass = 1.4;                              % [kg] Soil mass
c_C_TPH_g = c_C_TPH*mass;                % [g] Total Hydrocarbon mass
MW = 163.319;                            % [g/mol]
rho = 2650;                              % [kg/(m3)]
v_soil = mass/rho;                       % [m3] Soil volume
c_C = ((c_C_TPH_g)/MW)/v_soil;           % [mol/m3] Total Hydrocarbon
...
% ###
% conversion to TPH
G_C_TPH_f = G_C_converted/c_C;
G_C_TPH_f1 = G_C_converted1/c_C;
G_C_TPH_ekr = G_C_tot/c_C;
G_C_TPH_bkr = G_C_bio/c_C;
G_C_TPH_ekr_nr = G_C/c_C;
...
% Plot for figure 3
figure(3)
plot(x_scale,E_field(:,36000),'--','DisplayName', 'Voltage')
xlabel('Length (cm)');
ylabel('Voltage (V)');
...
% Plot for figure 4
figure(4)
plot(x_scale,E_field_dx(:,36000),'--','DisplayName', 'Voltage Gradient')
xlabel('Length (cm)');
